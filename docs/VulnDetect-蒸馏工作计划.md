...
> 版本 1.1 | 2026-07-07 | 预计工期 5-7 天（数据策略已更新为 REEF 为主）
...
│   入口：Phase 0 完成
│   出口：~4,000 条清洗后 REEF 数据 (reef_cleaned.jsonl)
│
├── Phase 2: Step 1 — LLM_message 修复 (Claude Opus)
│   入口：Phase 1 完成
│   出口：高质量漏洞描述数据 (reef_descriptions_train.jsonl)
│
├── Phase 3: Step 2 — 推理链蒸馏 (Claude Opus)
│   入口：Phase 1 完成 (可与 Phase 2 并行)
│   出口：7 部分安全审计推理链 (reef_reasoning_train.jsonl)
│
├── Phase 4: 基线训练 (组 A + 组 B)
│   入口：Phase 0 完成 (可与 Phase 1/2/3 并行)
│   出口：NVD-SFT 基线评测结果
│
├── Phase 5: 蒸馏训练 (组 C/D/E)
│   入口：Phase 2 + Phase 3 + Phase 4 完成
│   出口：多组蒸馏模型 + 评测结果
│
├── Phase 6: VulnBench 评测
│   入口：Phase 4 + Phase 5 完成
│   出口：VulnBench pass rate
│
└── Phase 7: 结果分析
    入口：全部评测完成
    出口：实验结论 + 下一步决策
